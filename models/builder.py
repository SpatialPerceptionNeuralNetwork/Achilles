...
def build(machine):
    ...
    outputs, ord_p, nn_out_labels, pretrained_variables, trainable_variables,end_points = _define_nn(INPUTS, IS_TRAINING)
    ...
    machine.labels_rgb = labels_rgb
    machine.labels_gt = labels_gt
...
def _define_nn(INPUTS, IS_TRAINING):
    ...
    def _set_aspp(feats, is_training):
        return _aspp.convolutional.scene_understanding_modular(
            feats=feats,
            num_classes=142,
            depth=256,
            atrous_rate=[6, 12],
    ...
    # from nets.test import d_conv
    # outputs, aspp_trainable_vars = d_conv.anonymous_1(feats, end_points, IS_TRAINING)

    # feats = _slim.conv2d(feats, 256, 5, 4)
    ...
    # outputs = _tf.image.resize_bilinear(feats, [385, 513])

    ord_p, nn_out_labels = _get_ordinal_values(outputs)

    pretrained_vars.extend(feats_extractor_pretrained_vars)

    return outputs, ord_p, nn_out_labels, pretrained_vars, trainable_vars, end_points
...
def _define_loss_fn(ord_p, groundtruth):
    def _get_ordinal_encoded_gt(groundtruth):
        ...
        # if N is None:
        #     N = _FLAGS.trainset_batch_size
        # groundtruth = _tf.reshape(groundtruth, shape=[None, H, W, 1])
        ...
        encoded_label = _tf.where(
            _tf.less(groundtruth, _FLAGS.GT_minima),
            -ones,
            encoded_label
        )
    ...
    global_mask = _tf.where(_tf.less(encoded_gt_labels, 0.0), _tf.zeros_like(encoded_gt_labels),
...
def _get_errors(pred, gt):
    pred = _tf.clip_by_value(_tf.cast(pred, _tf.float32), 1e-6, _FLAGS.GT_maxima)
